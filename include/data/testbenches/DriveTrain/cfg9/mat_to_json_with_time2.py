...
def VARIABLE_read_sub_array(self, hdr, copy=False):
    ...
    The modification only collect the variables with indices
    given in VARIABLE_INDICES 

    (The read_sub_array is heavily nested in scipy and inorder to
     avoid argument modifications of all these functions, 
    ...
    for key_index in VARIABLE_INDICES:
        VARIABLES[key_index] = list(arr[int(index_ar[key_index])-1, :])
...
if __name__ == '__main__':
    if (len(sys.argv) > 1):
        ...
        newdir = os.path.join(os.getcwd(), base_name)
